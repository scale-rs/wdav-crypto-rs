...
/// Environment variable name that contains private key generated by Deta.Space.
const ENV_DETA_KEY: &'static str = "";

/// Environment variable name that contains "salt", so that privileged users can't brute-force your
/// Deta.Space private key.
const ENV_SALT: &'static str = "SALT";

// Directory names here don't have a trailing slash. const TMP: &'static str = "/tmp"; // @TODO add
// if we use `const_format`
...
/// URL "segments". Warp requires them NOT to contain a slash.
...
async fn main() -> io::Result<()> {
    ...
    let dav_write_filter = {
        ...
        let dav_handler = dav_config(READ, read_write).build_handler();
        dav_server::warp::dav_handler(dav_handler)
    };
